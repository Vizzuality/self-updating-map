...
if (typeof(L.CartoDBLayer) === "undefined") {
  ...
  L.CartoDBLayer = L.Class.extend({
    ...
    _generateTileJson: function () {
      ...
      //HACK FOR MULTIPLE CNAMES
      //check if there are multiple domains CNAMES
      var grids = [urls.grid_url];
      
      if(urls.grid_url.indexOf("{s}") != -1) {
        grids = [                     
          urls.grid_url.replace(/\{s\}/g,"{s}"),
          urls.grid_url.replace(/\{s\}/g,"a"),
          urls.grid_url.replace(/\{s\}/g,"c"),
          urls.grid_url.replace(/\{s\}/g,"d"),
        ];
      }
    ...
    _checkTiles: function() {
      ...
      urls.tile_url = urls.tile_url.replace(/\{s\}/g,"a");
      urls.grid_url = urls.grid_url.replace(/\{s\}/g,"a");
